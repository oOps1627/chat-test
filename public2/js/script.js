...
$('.list-of-the-groups').on('click', function(e) {
    var target = e.target;
...
$('#my-group-list').on('click', function(e) {
    ...
    if(target.tagName == 'BUTTON') {
        ...
        if(target.classList.contains('left')) {
            ...
            var btnAllGroup = document.getElementById(groupId).children[0];
...
function updateAllGroupList(allGroups) {
    ...
    [].forEach.call(allGroups, function(group) {
        ...
        for (var i=0; i < ulMyGroups.children.length; i++) {
            ...
            if (id == group._id) {
                isMyGroup = ulMyGroups.children[i].children[0];
                break
            }
        }
        ...
        if (isMyGroup) { // if i have this button in the list of my groups
            ...
            button.className = "btn btn-default join"; // else create join btn in the list of all groups
        }
